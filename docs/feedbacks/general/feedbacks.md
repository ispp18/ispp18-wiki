...
### Clase 23/03/2018, Tarde

* En la próxima presentación se deben seguir presentando los problemas encontrados y las soluciones aplicadas. Para estas soluciones debemos desarrollar unas métricas empíricas que permitan conocer como de buena ha sido la solución aplicada. Para poder definir mejor lo buena que es la solución, se debe, del mismo modo, definir un objetivo,así, cuanto más cerca estemos del objetivo, más buena será nuestra métrica. 
* Una vez definidos los probmeas, soluciones y métricas aplicadas, se debe decir si las soluciones han resultado satisfactorias o no y en caso negativo, aportar nuevas soluciones.
* En la demo, es una buena idea aplicar zoom al video, de modo en que lo hizo Cocome o Adrenalive para que en todo momento y desde todo el aula se vea con claridad lo que va sucediendo en la aplicación.
* Es una buena idea seguir un hilo temporal en la presentación, del modo: Pasado(Que hemos hecho) --> Presente (Que estamos haciendo) --> Futuro (Que haremos en el próximo Sprint).
* En la demo, solo se debe mostrar el caso de uso más interesante, funcionalidades como el login no aportan nada a la presentación.
* Lo que está desplegado y se presenta debe estar funcionando al completo.
* En el video de la demo no es aconsejable perder tiempo rellenando formularios, ya que se hace un video, es posible saltar todos estos pasos y agilizar la presentación.
* No da buenas sensaciones que tras la reproducción del video se vean recomendaciones de youtube, es una buena práctica crear una dirección de correo profesional propia para la aplicación.
* En la presentación se deberían responder las siguientes preguntas: ¿Cuales han sido las medidas de rendimiento de los integrantes del grupo? ¿Ha habido problemas en cuanto al rendimiento individual? ¿Qué soluciones se han aplicado? ¿Han mejorado a las anteriores soluciones? ¿Cómo se mide si esas soluciones funcionan? ¿Qué métricas se usan para ello?
* No usar video de demostración en 4/3, pues se pierde mucho espacio aprovechable.
* En la demo, se debe poder apreciar quien está logado en cada momento y es aconsejable que siga un hilo argumental, como si contara una historia, de forma que así se mantenga la atención del público y se comprenda mejor el funcionamiento del sistema.
* Es muy recomendable mostrar en una sola gráfica el avance del estado del proyecto y los costes, para obtener de un sólo vistazo una idea del estado actual en el que nos encontramos,
* Cuando encontramos un problema, se debe medir el impacto que ese problema ha tenido sobre nuestro Sprint y en la presentación se debe decir con claridad, además de aclarar como ha sido el impacto frente a lo que se había estimado que podría haber sido al haberlo contemplado en los riesgos. Del mismo modo, hay que definir las medidas de contingencia que se tenian planeadas aplicar, las que se aplicaron finalmente y cual fue el resultado obtenido.
* Es el momento de comenzar el pilotaje, no puede ser difuso, se debe tener claro con quién se contacta, mediante qué medios, que se espera de ello y que se hará con la información obtenida.
* Gitflow para mejorar la metodología de trabajo con git.
* Todos los integrantes del grupo que forman parte del equipo de desarollo no pueden ser Ingenieros Software, hay que indicar las especializaciones (Ingeniero de requisitos, desarrollador frot-end, desarrollador back-end, desarrollador full-stack...)
* Las demostraciones siempre son susceptibles de fallos, por ello es bueno tener varios planes y alternativas (video, imágenes...)
* Para cada rendimiento/desempeño individual es buena idea mostrar las desviaciones entre lo estimado y lo que finalmente ha sucedido para así poder comprobar si la aplicación de las soluciones elegidas están resultando buenas.
* Es una buena idea hacer rendimientos comparados con el resto del equipo.
* En el plan de pilotaje es ESENCIAL que la información se extraiga del usuario que va a pagar por algun servicio en el sistema.
* En la presentación, no es necesario ir explicando persona por persona que se ha hecho, unicamente realizar una comparativa general poniendo las desviaciones que ha habido y el rendimiento de cada perosna explicando qué se ha hecho, qué problemas ha tenido esa persona y decir como se van a afrontar esos problemas.
* El sistema de pago es muy necesario para el pilotaje.
* En la exposición de la segunda semana del sprint, no es una retrospectiva de la última semana, si no de todo el sprint.
* Para todo progreso, definir cual habia sido la estimación del progreso y cuanto ha resultado ser finalmente.
* Si se dicen las tareas de cada integrante del grupo, no es una buena idea poner un tiempo general que las englobe a todas, mejor indicar un tiempo por tareas o, en su defecto, indicar el la proporcion de ese tiempo global que se ha invertido en cada tarea descrita (usar porcentajes, barras...). Siempre se debe poder ver de forma gráfica las desviaciones que han surgido en las tareas.
* ERRORES GRAVES QUE SE TENDRÁN MUY EN CUENTA EN LA PROXIMA SEMANA:
* Que haya cosas sin funcionar o no haya una url a la que acceder para poder probar la aplicación.
* No haber aplicado medidas de rendimiento o medidas completas para concretar mejoras.
* No tener una forma de ver el rendimiento comparado de las personas del equipo, para que se vea quien tiene mejor rendimiento.
* Tener mecanismos de recompensa y penalización en base al rendimiento de cada uno.
* Medir y la vez visualizar en conjunto el tiempo que se ha invertido sobre el tiempo total estimado tanto a nivel local de Sprint como a nivel global, comparado con el coste que se lleva invertido y el coste estimado, todo ello en una misma gráfica. Para ello, es buena idea realizar un burndown de las tareas y un burnup del coste.
* No se presenta al equipo dos veces, por ello si ya estamos presentando el desempeño y rendimiento individual, no se presenta al equipo al incio, pues ya hay una presentación implícita cuando se exponen los desempeños.
* Sprint que no tenga la aplicación funcionando es un Sprint fallido y automáticamente un no apto.
* Se debería ver siempre una visión global del proyecto, además de una local dentro del propio Sprint.
